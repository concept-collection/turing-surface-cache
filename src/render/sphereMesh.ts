...
export const fillPositions = (
  ...
  const n = nlat * nphi * 3;
  for (let p = 0; p < n; p++) {
    out[p] = (1 - morph) * sphereRef[p] + morph * coords[p];
  }
  const fillCap = (capIndex: number, rowIndex: number): void => {
    ...
    const p = capIndex * 3;
    out[p] = (1 - morph) * sphereRef[p] + (morph * x) / nphi;
    out[p + 1] = (1 - morph) * sphereRef[p + 1] + (morph * y) / nphi;
    out[p + 2] = (1 - morph) * sphereRef[p + 2] + (morph * z) / nphi;
  };
